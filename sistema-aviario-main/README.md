...
> **Configuração:** nenhuma senha, chave ou connection string fica no código. Tudo vem de variáveis de ambiente, a partir de um arquivo `.env` que **não** é versionado. O modelo está em `.env.example`, e o passo 1 abaixo explica como preencher.
>
> Para colocar no ar, veja **[DEPLOY.md](DEPLOY.md)** — configuração de produção, CORS, TLS, banco e checklist de publicação.
...
4. Abra o `.env` e preencha os valores marcados como `troque-...`. Para rodar **local**, o mínimo é:

```ini
ASPNETCORE_ENVIRONMENT=Development

MSSQL_SA_PASSWORD=<uma senha forte sua>
DB_APP_PASSWORD=<outra senha forte sua>
JWT_KEY=<texto aleatório de 32+ caracteres>
ESP32_SECRET_KEY=<texto aleatório de 24+ caracteres>
FRONTEND_BASE_URL=http://localhost:3000

# faz "docker compose up" já usar a sobreposição de desenvolvimento
COMPOSE_PATH_SEPARATOR=;
COMPOSE_FILE=docker-compose.yml;docker-compose.dev.yml
```

Para gerar valores aleatórios no PowerShell:
```powershell
[Convert]::ToBase64String((1..48 | ForEach-Object { Get-Random -Maximum 256 }))
```

> ⚠️ Nas senhas de banco (`MSSQL_SA_PASSWORD`, `DB_APP_PASSWORD`) use apenas letras, números e `! # % - _`. Os caracteres `$ ' "` quebram o script que cria o usuário do banco.
>
> O `.env` fica só na sua máquina e nunca deve ser enviado para o git.

5. Copie o arquivo de segredos do ESP32 e cole nele a **mesma** `ESP32_SECRET_KEY` que você acabou de definir no `.env`:
```powershell
Copy-Item esp32\src\secrets.example.h esp32\src\secrets.h
```

6. Certifique-se de que o aplicativo do **Docker Desktop** esteja aberto e rodando. Então, execute o comando abaixo para construir as imagens e iniciar todos os serviços em segundo plano:
```bash
docker compose up -d --build
```

> Se você preferiu não definir `COMPOSE_FILE` no `.env`, o comando de desenvolvimento é:
> ```bash
> docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build
> ```
> Sem a sobreposição, o `docker-compose.yml` sozinho sobe em modo **produção**: sem Swagger, sem porta da API no host.

A primeira subida cria o banco e um usuário de aplicação dedicado (o backend não se conecta como `sa`) e aplica as migrations. Assim que o processo terminar, seus ambientes estarão acessíveis nos seguintes endereços:
* **Frontend:** `http://localhost:3000`
* **Backend Swagger** `http://localhost:5000/swagger`

> Todas as portas ficam publicadas apenas em `127.0.0.1` — acessíveis da sua máquina, não da rede. A porta `1433` do SQL Server só é publicada em desenvolvimento; em produção ela não sai da rede interna do Docker.
...
3. No campo `X-Secret-Key`, cole a chave que você definiu em `ESP32_SECRET_KEY` no seu `.env` (a mesma que está em `esp32/src/secrets.h`).
...
Este projeto **não guarda segredo nenhum em arquivo versionado**. Antes de publicar em um servidor, leia **[DEPLOY.md](DEPLOY.md)**, que cobre:

* rotação das credenciais que ficaram expostas no histórico do repositório;
* `.env` de produção e validação da configuração no startup da API;
* CORS restrito por lista de origens;
* SQL Server sem porta publicada e usuário de aplicação sem privilégio de servidor;
* TLS na borda e checklist final de publicação.